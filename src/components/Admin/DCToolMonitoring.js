...
// // code with multiple bolt tightening and tightening logic and mqtt from frontend.
...
// // code with multiple bolt tightening, tightening logic, DB insert and mqtt from frontend
// import React, { useEffect, useState, useRef } from "react";
// import { useParams } from "react-router-dom";
// import mqtt from "mqtt";
...
const DCToolHMI = () => {
  ...
  const prePitchTopic = "PrePitch";
  const engineTopic = `ST${stageNumber}_EngineNumber`;
  const lineStatusTopic = `ST${stageNumber}_Line_Status`;
  ...
  useEffect(() => {
    ...
    const fetchSignals = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/mqtt-signal/by-stage-no/${stageNumber}`);
        const json = await res.json();
  ...
  useEffect(() => {
    ...
    const client = mqtt.connect("ws://192.168.1.26:9001", {
      username: "mqttuser",
      password: "Theta@123",
      reconnectPeriod: 3000,
      clean: true,
    ...
    client.on("connect", () => {
      ...
      client.subscribe(prePitchTopic);
      client.subscribe(lineStatusTopic);
    });
    ...
    client.on("message", (topic, message) => {
      ...
      /* ===== LINE STATUS ===== */
      if (topic === lineStatusTopic) {
        const value = Number(payload);

        if (!isNaN(value)) {
          if (value === 1) {
            setLineStatus("INTERLOCKED");
          } else if (value === 0) {
            setLineStatus("BYPASSED");
          }
        }
      }
